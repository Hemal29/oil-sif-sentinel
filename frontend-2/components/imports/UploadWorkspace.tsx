    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
          ...
          <div className="panel-body">
            <form onSubmit={onUpload} className="space-y-4">
              <div
                className={
                  dragOver
                    ? "rounded-2xl border-2 border-[#C8791A] bg-[#FCF0DE] p-8 text-center sm:p-10"
                    : "rounded-2xl border-2 border-dashed border-[#D9D0BE] bg-[#FAF8F4] p-8 text-center transition-colors hover:border-[#9BA3AF] sm:p-10"
                }
              ...
              >
                ...
                <label htmlFor={inputId} className="cursor-pointer">
                  <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl border border-[#E7E1D4] bg-white text-[#6B7385]">
                    <ImportsIcon className="h-5 w-5" />
                  </div>
                  <p className="mt-3 text-[14px] font-semibold text-[#14283F]">Drag &amp; drop your file here</p>
                  <p className="mt-1 text-[13px] text-[#6B7385]">
                    or <span className="font-semibold text-[#C8791A] underline">Choose File</span>
                  </p>
                </label>
              </div>
              {file ? (
                <div className="flex items-center justify-between gap-3 rounded-xl border border-[#E7E1D4] bg-[#FAF8F4] px-4 py-3">
                  <p className="min-w-0 truncate text-[13px] text-[#14283F]">
                    <strong className="font-semibold">{file.name}</strong> <span className="text-[#6B7385]">({formatBytes(file.size)})</span>
                  </p>
                  <button type="button" onClick={() => setFile(null)} aria-label="Remove selected file" className="rounded-lg p-1.5 text-[#6B7385] hover:bg-[#E7E1D4]">
        ...
        <aside className="panel h-fit lg:col-span-2" aria-label="Instructions">
          ...
          <div className="panel-body">
            <ol className="space-y-3.5 text-[13px] leading-relaxed text-[#4C5566]">
              {[
                ...
                <li key={i} className="flex gap-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#14283F] text-[11px] font-bold text-white">{i + 1}</span>
                  <span>{step}</span>
                </li>
              ))}
            </ol>
      ...
      {result ? (
        <section className="panel" aria-label="Import summary">
          <div className="panel-header">
            ...
            <span className="font-mono text-[11px] text-[#9BA3AF]">{result.batchId}</span>
          </div>
          <div className="panel-body">
            <p className="text-[13.5px] text-[#14283F]">{result.message}</p>
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-[#E7E1D4] bg-[#FAF8F4] p-4 text-center">
                <p className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">Total rows</p>
                <p className="mt-1 text-[26px] font-bold tabular-nums text-[#14283F]">{result.totalRows}</p>
              </div>
            ...
            {result.errors.length > 0 ? (
              <div className="mt-5">
                <h3 className="text-[13.5px] font-bold text-[#14283F]">
                  Row-level errors ({result.errors.length}
                  {result.errorsTruncated ? ` shown, +${result.additionalErrors} more` : ""})
                </h3>
                <div className="mt-2 overflow-x-auto rounded-xl border border-[#E7E1D4]">
