          ...
          <>
            {official.length > 0 ? (
              <section aria-label="Official rules">
                <h2 className="mb-3 text-[15px] font-bold tracking-tight text-[#14283F]">
    ...
    <article className="panel flex flex-col p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="text-[14.5px] font-bold tracking-tight text-[#14283F]">{rule.name}</h3>
          <p className="mt-1 font-mono text-[12px] text-[#6B7385]">{rule.code}</p>
        </div>
        {rule.isPrototype ? <PrototypeBadge /> : <OfficialBadge />}
      </div>
      {rule.description ? <p className="mt-3 line-clamp-3 text-[13px] leading-relaxed text-[#4C5566]">{rule.description}</p> : null}
      <div className="mt-4 flex items-center justify-between border-t border-[#EFEADF] pt-3.5">
