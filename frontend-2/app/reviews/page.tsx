...
function RouteChip({ route }: { route: string | null | undefined }) {
  if (!route) return <span className="text-[12px] text-[#9BA3AF]">—</span>;
    ...
    <div className="space-y-5">
      ...
      <div className="filter-bar" role="search" aria-label="Review filters">
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">AI Route</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">Status</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">Type</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">Site</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">SIF</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">From</span>
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="field" aria-label="From date" />
        </label>
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">To</span>
            ...
            <table className="data-table min-w-[1180px]">
              ...
              <tbody>
                {items.map((r: any) => {
                  ...
                  return (
                    <tr key={r.id} className={priority ? "border-l-2 border-l-[#C8791A] bg-[#FCF0DE]/50" : undefined}>
                      <td className="whitespace-nowrap font-mono text-[12.5px] font-semibold">
                        <Link href={`/reports/${r.id}`} className="inline-flex items-center gap-2 text-[#C8791A] hover:underline">
                          {priority ? <span className="h-2 w-2 shrink-0 rounded-full bg-[#C8791A]" aria-label="Priority" /> : null}
                          {r.reportNumber}
                        </Link>
                      </td>
                      <td className="whitespace-nowrap tabular-nums text-[12.5px] text-[#6B7385]">{r.date}</td>
                      <td>
                      ...
                      <td className="tabular-nums text-[13px] font-semibold">
                        {r.aiAnalysis?.riskScore != null ? `${(r.aiAnalysis.riskScore * 100).toFixed(0)}%` : <span className="font-normal text-[#9BA3AF]">—</span>}
                      </td>
                      <td>{r.aiAnalysis ? <SifBadge value={r.aiAnalysis.sifPotential} /> : <span className="text-[12px] text-[#9BA3AF]">—</span>}</td>
                      <td>
                      ...
                      <td className="whitespace-nowrap">
                        {reviewer ? (
                          ...
                          <Link href={`/reports/${r.id}`} className="text-[12.5px] font-semibold text-[#C8791A] hover:underline">
                            View →
                          </Link>
                        )}{" "}
                        <Link href={`/reports/${r.id}#activity`} className="px-1 text-[12px] font-semibold text-[#C8791A] hover:underline">
                          Activity
                        </Link>
                      </td>
                    </tr>
