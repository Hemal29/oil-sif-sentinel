...
function IntelligenceList({ items, emptyText, barColor }: { items: TopCount[]; emptyText: string; barColor: string }) {
  if (items.length === 0) return <p className="py-4 text-center text-[13px] text-[#9BA3AF]">{emptyText}</p>;
      ...
      {items.slice(0, 6).map((i) => (
        <li key={i.name}>
          <div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]">
            <span className="min-w-0 truncate font-medium text-[#14283F]">{i.name}</span>
            <span className="shrink-0 font-bold tabular-nums text-[#14283F]">{i.count.toLocaleString()}</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-[#F2EEE5]" role="img" aria-label={`${i.name}: ${i.count}`}>
  ...
  return (
    <div className="space-y-7">
        <PageHeader
          ...
          actions={
            <div className="flex flex-wrap items-center gap-1.5 rounded-xl border border-[#E7E1D4] bg-white p-1.5 shadow-[0_1px_2px_rgba(20,40,63,0.05)]" role="group" aria-label="Reporting period">
              {PRESETS.map((p) => (
                <button
                  key={p.value}
                  onClick={() => {
                  ...
                  className={`rounded-lg px-3 py-1.5 text-[12.5px] font-semibold transition-colors ${
                    preset === p.value && !useCustom ? "bg-[#14283F] text-white" : "text-[#6B7385] hover:bg-[#F2EEE5] hover:text-[#14283F]"
                  }`}
                >
                  {p.label}
                </button>
              ))}
              <span className="mx-1 hidden h-5 w-px bg-[#E7E1D4] sm:block" aria-hidden="true" />
              ...
              <span className="text-[12px] text-[#9BA3AF]">to</span>
          ...
          <>
              ...
              <div className="panel h-full">
                ...
                <div className="panel-body">
                  {routeCounts.length === 0 ? (
                    <p className="py-8 text-center text-[13px] text-[#9BA3AF]">No AI screening outcomes yet.</p>
                  ) : (
                    <ul className="space-y-4">
                      {routeCounts.map((r) => (
                        <li key={r.name}>
                          <div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]">
                            <span className="font-semibold uppercase tracking-wide text-[#14283F]">{r.name.replace("_", " ")}</span>
                            <span className="shrink-0 font-bold tabular-nums text-[#14283F]">
                              {r.count.toLocaleString()}
                              <span className="ml-2 font-medium text-[#6B7385]">{((r.count / routeTotal) * 100).toFixed(0)}%</span>
                            </span>
                          </div>
                          <div className="h-2 overflow-hidden rounded-full bg-[#E7E1D4]" role="img" aria-label={`${r.name}: ${r.count} reports`}>
                            <div className="h-full rounded-full bg-[#C8791A]" style={{ width: `${(r.count / routeTotal) * 100}%` }} />
                          </div>
                        </li>
                      ))}
                    </ul>
            ...
            <section aria-label="Safety intelligence">
              <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
                <div>
                  <h2 className="text-[20px] font-bold tracking-tight text-[#14283F]">Safety Intelligence</h2>
                  <p className="mt-1 text-[13px] text-[#6B7385]">Precursor signals detected across validated safety reports.</p>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div className="panel">
                  ...
                  <div className="panel-body">
                    <IntelligenceList items={precursor} emptyText="No precursor signals for this period." barColor="#C8791A" />
                  </div>
                </div>
                <div className="panel">
                  <div className="panel-header">
                  ...
                  <div className="panel-body">
                    <IntelligenceList items={energies} emptyText="No hazard energy data for this period." barColor="#C8791A" />
                  </div>
                </div>
                <div className="panel">
                  <div className="panel-header">
                  ...
                  <div className="panel-body">
                    <IntelligenceList items={barriers} emptyText="No barrier data for this period." barColor="#C8791A" />
                  </div>
                </div>
              </div>
            </section>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
              <div className="panel xl:col-span-1">
                <div className="panel-header">
                  ...
                  <Link href="/reviews" className="inline-flex items-center gap-1 text-[12.5px] font-semibold text-[#C8791A] hover:underline">
                    Open <span aria-hidden="true">→</span>
                  </Link>
                </div>
                <div className="p-0">
                  {pending.length === 0 ? (
                    <p className="px-5 py-8 text-center text-[13px] text-[#9BA3AF]">Queue is clear. No reports awaiting review.</p>
                  ) : (
                    <ul className="divide-y divide-[#EFEADF]">
                      {pending.slice(0, 6).map((r) => (
                        <li key={r.reportId}>
                          <Link href={`/reports/${r.reportId}`} className="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-[#FAF8F4]">
                            <span className="min-w-0">
                              <span className="block truncate font-mono text-[12.5px] font-semibold text-[#C8791A]">{r.reportNumber}</span>
                              <span className="block truncate text-[12px] text-[#6B7385]">{r.activity}</span>
                            </span>
              ...
              <div className="panel overflow-hidden xl:col-span-2">
                <div className="panel-header">
                  ...
                  <Link href="/reports" className="inline-flex items-center gap-1 text-[12.5px] font-semibold text-[#C8791A] hover:underline">
                ...
                <div className="overflow-x-auto">
                  <table className="data-table min-w-[760px]">
                    ...
                    <tbody>
                      {recent.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-5 py-8 text-center text-[13px] text-[#9BA3AF]">
                        ...
                        recent.slice(0, 8).map((r) => (
                          <tr key={r.analysisId ?? r.reportId}>
                            <td>
                              <Link href={`/reports/${r.reportId}`} className="font-mono text-[12.5px] font-semibold text-[#C8791A] hover:underline">
                                {r.reportNumber}
                              </Link>
                              <span className="block text-[11.5px] text-[#9BA3AF]">{r.date}</span>
                            </td>
            ...
            <p className="rounded-xl border border-[#E7E1D4] bg-white px-4 py-3 text-[11.5px] leading-relaxed text-[#6B7385]">
