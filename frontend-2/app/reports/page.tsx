...
function RouteCell({ report }: { report: SafetyReport }) {
  ...
  if (!a || a.analysisStatus !== "COMPLETED" || !a.route) return <span className="text-[12px] text-[#9BA3AF]">—</span>;
    ...
    <div className="space-y-5">
        ...
        <form onSubmit={onSubmit} className="filter-bar" aria-label="Report filters">
          ...
          <label className="flex flex-col gap-1.5">
            ...
            <select value={site} onChange={(e) => setSite(e.target.value)} className="field min-w-[150px]" aria-label="Filter by site">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wide text-[#6B7385]">Activity</span>
            <input type="text" placeholder="Activity…" value={activity} onChange={(e) => setActivity(e.target.value)} className="field min-w-[140px]" aria-label="Filter by activity" />
          </label>
          <label className="flex flex-col gap-1.5">
                  ...
                  {items.map((r) => (
                    <tr key={r.id}>
                      <td className="whitespace-nowrap font-mono text-[12.5px] font-semibold">
                        <Link href={`/reports/${r.id}`} className="text-[#C8791A] hover:underline">
                          {r.reportNumber}
                        </Link>
                      </td>
                      <td className="max-w-[260px] truncate text-[13px] text-[#4C5566]" title={r.description}>
                        {r.description}
                      </td>
                      <td className="max-w-[130px] truncate text-[13px]">
                      ...
                      <td className="max-w-[150px] truncate text-[13px]">{r.site ? `${r.site.name}` : "—"}</td>
                      <td className="max-w-[170px] truncate text-[13px]">{r.activity}</td>
                      <td>
                        <SifCell report={r} />
                      </td>
                      <td>
