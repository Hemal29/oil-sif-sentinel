        ...
        {!user ? (
          ...
          <div className="panel overflow-hidden">
            <ul className="divide-y divide-[#EDF1F6]">
              {items.map((n) => (
                <li key={n.id}>
                  <button onClick={() => handleClick(n)} className={cn("flex w-full gap-3.5 px-4 py-3.5 text-left transition-colors hover:bg-[#F8FAFC] sm:px-5", !n.isRead ? "bg-[#FEF3E7]" : "bg-white")}>
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#FDE8C8] text-[#D97706]" aria-hidden="true">
                      <BellIcon className="h-4 w-4" />
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="flex flex-wrap items-center gap-2">
                        <span className={cn("text-[13.5px]", !n.isRead ? "font-bold text-[#0F172A]" : "font-medium text-[#334155]")}>{n.title}</span>
                        {!n.isRead ? <span className="badge badge-info">New</span> : <span className="badge badge-neutral">Read</span>}
                      </span>
                      <span className="mt-0.5 block text-[13px] leading-relaxed text-[#475569]">{n.message}</span>
            ...
            <div className="flex items-center justify-between border-t border-[#E2E8F0] px-4 py-3.5 sm:px-5">
