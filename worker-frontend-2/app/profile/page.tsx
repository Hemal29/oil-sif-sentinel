...
function FieldRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex flex-col gap-1 border-b border-[#EDF1F6] py-3 last:border-0 sm:flex-row sm:items-center sm:justify-between">
      <span className="text-[11px] font-bold uppercase tracking-wide text-[#64748B]">{label}</span>
      <span className={`text-[13px] font-medium text-[#102A43] ${mono ? "font-mono" : ""}`}>{value || "—"}</span>
    </div>
  );
}
          ...
          <>
            ...
            <section className="panel">
              <div className="panel-header">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-wide text-[#94A3B8]">Real data from MySQL</span>
              </div>
            ...
            <section className="panel">
              <div className="panel-header">
                ...
                <span className="text-[11px] text-[#94A3B8]">Update your name and mobile number</span>
              </div>
              <div className="panel-body">
                <form onSubmit={onSaveProfile} noValidate className="space-y-4">
                  {editErrors.form ? <p className="rounded-[4px] border border-[#FECACA] bg-[#FEF2F2] px-4 py-2.5 text-[13px] font-medium text-[#991B1B]" role="alert">{editErrors.form}</p> : null}
                  {saveSuccess ? <p className="rounded-[4px] border border-[#BBF7D0] bg-[#F0FDF4] px-4 py-2.5 text-[13px] font-medium text-[#15803D]" role="status">{saveSuccess}</p> : null}
                  <label className="block">
                    <span className="mb-1 block text-[12px] font-semibold text-[#0F172A]">Full Name <span className="text-[#991B1B]">*</span></span>
                    <input id="field-name" value={editName} onChange={(e) => setEditName(e.target.value)} className={`field w-full ${editErrors.name ? "!border-[#DC2626]" : ""}`} aria-invalid={!!editErrors.name} aria-describedby={editErrors.name ? "err-name" : undefined} />
                    {editErrors.name ? <p id="err-name" className="mt-1 text-[11px] text-[#991B1B]">{editErrors.name}</p> : null}
                  </label>
                  <label className="block">
                    ...
                    <input id="field-mobile" value={editMobile} onChange={(e) => setEditMobile(e.target.value)} placeholder="9876543210" className={`field w-full ${editErrors.mobile ? "!border-[#DC2626]" : ""}`} aria-invalid={!!editErrors.mobile} aria-describedby={editErrors.mobile ? "err-mobile" : undefined} />
                    {editErrors.mobile ? <p id="err-mobile" className="mt-1 text-[11px] text-[#991B1B]">{editErrors.mobile}</p> : null}
                  </label>
                  <p className="text-[11px] text-[#94A3B8]">Email, Employee ID, Role and Status cannot be changed from this page.</p>
            ...
            <section className="panel">
              <div className="panel-header">
                ...
                <span className="text-[11px] text-[#94A3B8]">Change your password</span>
              </div>
              <div className="panel-body">
                <form onSubmit={onChangePassword} noValidate className="space-y-4">
                  {pwErrors.form ? <p className="rounded-[4px] border border-[#FECACA] bg-[#FEF2F2] px-4 py-2.5 text-[13px] font-medium text-[#991B1B]" role="alert">{pwErrors.form}</p> : null}
                  {pwSuccess ? <p className="rounded-[4px] border border-[#BBF7D0] bg-[#F0FDF4] px-4 py-2.5 text-[13px] font-medium text-[#15803D]" role="status">{pwSuccess}</p> : null}
                  <label className="block">
                    <span className="mb-1 block text-[12px] font-semibold text-[#0F172A]">Current Password <span className="text-[#991B1B]">*</span></span>
                    <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} autoComplete="current-password" className={`field w-full ${pwErrors.currentPassword ? "!border-[#DC2626]" : ""}`} aria-invalid={!!pwErrors.currentPassword} aria-describedby={pwErrors.currentPassword ? "err-currentPassword" : undefined} />
                    {pwErrors.currentPassword ? <p id="err-currentPassword" className="mt-1 text-[11px] text-[#991B1B]">{pwErrors.currentPassword}</p> : null}
                  </label>
                  <label className="block">
                    <span className="mb-1 block text-[12px] font-semibold text-[#0F172A]">New Password <span className="text-[#991B1B]">*</span></span>
                    <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} autoComplete="new-password" placeholder="At least 8 characters" className={`field w-full ${pwErrors.newPassword ? "!border-[#DC2626]" : ""}`} aria-invalid={!!pwErrors.newPassword} aria-describedby={pwErrors.newPassword ? "err-newPassword" : undefined} />
                    {pwErrors.newPassword ? <p id="err-newPassword" className="mt-1 text-[11px] text-[#991B1B]">{pwErrors.newPassword}</p> : null}
                  </label>
                  <label className="block">
                    <span className="mb-1 block text-[12px] font-semibold text-[#0F172A]">Confirm New Password <span className="text-[#991B1B]">*</span></span>
                    <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} autoComplete="new-password" placeholder="Repeat new password" className={`field w-full ${pwErrors.confirmPassword ? "!border-[#DC2626]" : ""}`} aria-invalid={!!pwErrors.confirmPassword} aria-describedby={pwErrors.confirmPassword ? "err-confirmPassword" : undefined} />
                    {pwErrors.confirmPassword ? <p id="err-confirmPassword" className="mt-1 text-[11px] text-[#991B1B]">{pwErrors.confirmPassword}</p> : null}
                  </label>
