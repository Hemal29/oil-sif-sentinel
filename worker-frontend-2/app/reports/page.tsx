...
function WorkerReportsContent() {
  ...
  return (
      <div className="space-y-4 sm:space-y-5">
        ...
        <div className="flex gap-1 overflow-x-auto rounded-[12px] border border-[#E2E8F0] bg-white p-1.5 shadow-[0_1px_2px_rgba(15,23,42,0.05)]" role="tablist" aria-label="Report views">
          {(["all", "drafts", "submitted", "closed"] as Tab[]).map((t) => (
            <button
              ...
              aria-selected={tab === t}
              onClick={() => setTab(t)}
              className={`whitespace-nowrap rounded-lg px-4 py-2 text-[13.5px] font-semibold transition-colors ${tab === t ? "bg-[#0F172A] text-white" : "text-[#64748B] hover:bg-[#F1F5F9] hover:text-[#0F172A]"}`}
            >
              ...
              {drafts.map((d) => (
                <article key={d.id} className="panel flex flex-col p-5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[11.5px] tabular-nums text-[#94A3B8]">
                      {d.updatedAt ? new Date(d.updatedAt).toLocaleString(undefined, { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : ""}
                    </span>
                  </div>
                  <p className="mt-2.5 text-[14px] font-bold text-[#0F172A]">{d.activity || d.reportType?.replace(/_/g, " ") || "Untitled draft"}</p>
                  <p className="mt-1 line-clamp-2 text-[13px] leading-relaxed text-[#64748B]">{d.description || "No description yet."}</p>
                  <div className="mt-4 flex gap-2 border-t border-[#EDF1F6] pt-3.5">
            ...
            {loading ? (
              ...
              <>
                ...
                <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:hidden">
                  {items.map((r) => (
                    <li key={r.id}>
                      <Link href={`/reports/${r.id}`} className="block rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.05)] transition-colors hover:border-[#FBD8A0]">
                        <p className="font-mono text-[12.5px] font-bold text-[#D97706]">{r.reportNumber}</p>
                        <p className="mt-1.5 text-[14px] font-bold text-[#0F172A]">
                          {r.reportType.replace(/_/g, " ")} · {r.activity}
                        </p>
                        <p className="mt-1 line-clamp-2 text-[13px] leading-relaxed text-[#64748B]">“{r.description}”</p>
                        ...
                        {items.map((r) => (
                          <tr key={r.id}>
                            <td className="whitespace-nowrap font-mono text-[12.5px] font-semibold">
                              <Link href={`/reports/${r.id}`} className="text-[#D97706] hover:underline">
                                {r.reportNumber}
                              </Link>
                            </td>
                            <td>
                            ...
                            <td className="whitespace-nowrap tabular-nums text-[#64748B]">{formatDate(r.date)}</td>
                            <td>
                              <StatusBadge value={r.status} />
                            </td>
                            <td>
      ...
      {deleteTarget ? (
        <Modal title="Delete draft?" description="This unfinished report will be permanently removed." onClose={() => setDeleteTarget(null)}>
          <div className="flex justify-end gap-2">
            ...
            <button onClick={handleDeleteDraft} disabled={deleting} className="button-primary bg-[#DC2626] border-[#DC2626] hover:bg-[#B91C1C] disabled:opacity-50">
