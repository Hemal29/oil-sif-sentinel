  ...
  return (
    <WorkerShell title="Dashboard">
      <div className="space-y-5 sm:space-y-6">
        {/* HERO — Industrial: charcoal panel, teal hairline accent */}
        <section className="overflow-hidden rounded-[22px] border border-white/5 bg-[#151C26] shadow-[0_1px_2px_rgba(0,0,0,0.35),0_8px_24px_rgba(0,0,0,0.25)]" aria-label="Welcome">
          <div className="relative grid grid-cols-1 lg:h-[375px] lg:min-h-[360px] lg:max-h-[390px] lg:grid-cols-[55%_45%] lg:overflow-hidden">
            <div className="flex flex-col justify-center px-5 py-8 sm:px-8 lg:px-9 lg:py-7">
              <p className="mb-[18px] font-mono text-[12px] font-bold uppercase tracking-[0.18em] text-[#00E5FF]">Worker Safety Portal</p>
              <h1
                title={`${greeting()}, ${firstName}`}
                className="max-w-full font-display text-[34px] font-extrabold leading-[1.08] tracking-[-0.02em] text-white sm:text-[40px] lg:overflow-hidden lg:text-ellipsis lg:whitespace-nowrap lg:text-[48px] lg:leading-[1.05] lg:tracking-[-0.03em]"
              >
                {greeting()}
                {firstName ? `, ${firstName}` : ""} <span aria-hidden="true">👋</span>
              </h1>
              <p className="mt-3 max-w-[480px] text-[16px] leading-[1.5] text-white/50 sm:text-[17px] lg:text-[18px]">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-nowrap sm:items-center sm:gap-[14px]">
                ...
                <Link
                  href="/reports"
                  className="button-secondary h-[52px] w-full whitespace-nowrap border-white/10 bg-white/[0.04] px-5 text-[15px] text-white hover:bg-white/[0.07] sm:h-[54px] sm:w-auto sm:min-w-[185px]"
                >
            ...
            {/* vertical divider */}
            <span className="absolute bottom-[15%] left-[55%] top-[15%] hidden w-px bg-white/5 lg:block" aria-hidden="true" />
            <div className="relative min-h-[240px] overflow-hidden sm:min-h-[280px] lg:h-[375px] lg:min-h-[360px] lg:max-h-[390px]">
              <Image
                ...
                className="object-cover object-[65%_center] brightness-[0.92]"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A0D12]/40 via-transparent to-transparent sm:bg-none" aria-hidden="true" />
              <div
                className="absolute inset-y-0 left-0 w-[64%] bg-gradient-to-r from-[#151C26] via-[#151C26]/85 to-transparent"
                aria-hidden="true"
              />
              <div className="relative flex h-full min-h-[240px] flex-col p-5 sm:min-h-[280px] sm:p-7 lg:h-[375px] lg:min-h-[360px] lg:p-7">
                <p className="pr-1 text-right font-mono text-[11px] font-bold uppercase leading-[1.7] tracking-[0.25em] text-white/30 lg:text-[12px]">
                  Safe People
                  <br />
                  Stronger
                  <br />
                ...
                <div className="mt-auto max-w-[250px] pt-6 sm:max-w-[270px]">
                  <p className="font-display text-[30px] font-extrabold leading-[1.08] tracking-tight text-white sm:text-[33px] lg:text-[34px] lg:leading-[1.05]">
                    Small Observations
                    <br />
                    <span className="text-[#00E5FF]">Make a Safer Tomorrow</span>
                  </p>
                  <span className="mt-3 block h-[3px] w-9 rounded-full bg-[#00E5FF] shadow-[0_0_8px_rgba(0,229,255,0.5)]" aria-hidden="true" />
                  <p className="mt-2.5 text-[14px] font-medium text-white/45 sm:text-[15px]">See it. Report it. Prevent it.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:gap-5 xl:grid-cols-5">
              <section className="panel overflow-hidden xl:col-span-3" aria-label="Recent reports">
                <div className="panel-header">
                  ...
                  <Link href="/reports" className="text-[12.5px] font-semibold text-[#00E5FF] hover:underline">
                    View all →
                  </Link>
                </div>
                {recent.length === 0 ? (
                  ...
                  <ul className="divide-y divide-white/5">
                    {recent.map((r) => (
                      <li key={r.id}>
                        <Link href={`/reports/${r.id}`} className="block px-5 py-4 transition-colors hover:bg-white/[0.02]">
                          <p className="font-mono text-[12px] font-bold text-[#00E5FF]">{r.reportNumber}</p>
                          <p className="mt-1 text-[13.5px] font-semibold text-white">
                            {r.reportType.replace(/_/g, " ")} · {r.activity}
                          </p>
                          <p className="mt-0.5 line-clamp-1 text-[13px] text-white/45">{r.description}</p>
              ...
              <div className="space-y-4 sm:space-y-5 xl:col-span-2">
                  ...
                  <div className="panel-body">
                    {drafts.length === 0 ? (
                      <div className="flex gap-3">
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/[0.04] text-white/30 ring-1 ring-white/5">
                          <InboxIcon className="h-5 w-5" />
                        </span>
                        <div>
                          <p className="text-[13.5px] font-bold text-white">No saved drafts</p>
                          <p className="mt-0.5 text-[12.5px] text-white/45">Your unfinished reports will appear here.</p>
                        </div>
                      </div>
                    ) : (
                      <ul className="space-y-3">
                        {drafts.map((d) => (
                          <li key={d.id} className="rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3 transition-colors hover:border-white/10">
                            <p className="line-clamp-1 text-[13.5px] font-semibold text-white">
                              {d.activity || d.reportType?.replace(/_/g, " ") || "Untitled draft"}
                            </p>
                            <p className="mt-0.5 font-mono text-[12px] text-white/35">{formatSaved(d.updatedAt)}</p>
                            <Link href={`/reports/new?draftId=${d.id}`} className="mt-1.5 inline-block text-[12.5px] font-semibold text-[#00E5FF] hover:underline">
                              Continue →
                            </Link>
                          </li>
                        ))}
                ...
                <section className="panel" aria-label="Notifications">
                  <div className="panel-header">
                    ...
                    <Link href="/notifications" className="text-[12.5px] font-semibold text-[#00E5FF] hover:underline">
                      View all →
                    </Link>
                  </div>
                  <div className="panel-body">
                    {notes.length === 0 ? (
                      <div className="flex gap-3">
                        ...
                        <div>
                          <p className="text-[13.5px] font-bold text-white">You&apos;re all caught up</p>
                          <p className="mt-0.5 text-[12.5px] text-white/45">No new notifications.</p>
                        </div>
                      </div>
                    ) : (
                        ...
                        {notes.slice(0, 3).map((n) => (
                          <li key={n.id}>
                            <Link
                              ...
                              className={`block rounded-xl px-3.5 py-2.5 transition-colors hover:bg-white/[0.03] ${!n.isRead ? "bg-[rgba(0,229,255,0.06)] border border-[rgba(0,229,255,0.12)]" : "border border-transparent"}`}
                            >
                              <p className="flex items-center gap-2 text-[13px] font-semibold text-white">
                                {!n.isRead ? <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#00E5FF] shadow-[0_0_6px_rgba(0,229,255,0.6)]" aria-label="Unread" /> : null}
                                <span className="line-clamp-1">{n.title}</span>
                              </p>
                              <p className="mt-0.5 line-clamp-1 pl-3.5 text-[12px] text-white/45">{n.message}</p>
                              <p className="mt-0.5 pl-3.5 font-mono text-[11px] tabular-nums text-white/30">{formatAgo(n.createdAt)}</p>
                            </Link>
                          </li>
                        ))}
                      </ul>
            ...
            {summary.needsInformation > 0 ? (
              <div className="flex gap-3 rounded-2xl border border-[rgba(0,229,255,0.18)] bg-[rgba(0,229,255,0.06)] p-4 sm:p-5" role="status">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[13px] text-white/60">The HSE team requested more information. Open the report to read their request.</p>
                  <Link href="/reports" className="mt-1.5 inline-block text-[13px] font-semibold text-[#00E5FF] hover:underline">
                    View my reports →
                  </Link>
                </div>
              </div>
