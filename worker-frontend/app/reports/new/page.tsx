...
const labelCls = "mb-1.5 block text-[13.5px] font-semibold text-white";
const errorCls = "mt-1.5 text-[13px] font-medium text-[#FCA5A5]";
      ...
      <WorkerShell title="New Report">
        <div className="mx-auto w-full max-w-[720px]">
          <section className="panel p-6 text-center sm:p-10" aria-live="polite">
            <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[rgba(0,229,255,0.12)] text-[#00E5FF] ring-1 ring-[#00E5FF]/20">
            ...
            <p className="eyebrow mt-4 text-[#00E5FF]">Submission received</p>
            <h1 className="page-title mt-2 text-white">Report Submitted</h1>
            <p className="mx-auto mt-2 max-w-[52ch] text-[14px] text-white/50">Your safety observation has been submitted successfully.</p>
            <dl className="mx-auto mt-5 max-w-[420px] rounded-2xl border border-white/5 bg-[#0A0D12] px-5 py-4 text-left">
              <div className="flex items-center justify-between gap-4">
                <dt className="text-[13px] text-white/40">Report Number</dt>
                <dd className="font-mono text-[14px] font-bold text-white">{created.reportNumber}</dd>
              </div>
              <div className="mt-2 flex items-center justify-between gap-4 border-t border-white/5 pt-2">
                <dt className="text-[13px] text-white/40">Status</dt>
                <dd className="text-[13px] font-semibold text-white">Submitted</dd>
              </div>
            </dl>
            <div className="mx-auto mt-5 max-w-[420px] rounded-2xl border border-white/5 bg-white/[0.02] px-5 py-4 text-left">
              <p className="text-[13.5px] font-bold text-white">What happens next?</p>
              <p className="mt-1 text-[13px] leading-relaxed text-white/50">The HSE team will review your report. You can track its progress under My Reports.</p>
            </div>
  ...
  return (
    <WorkerShell title="New Report">
      <div className="mx-auto w-full max-w-[720px]">
        ...
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/5 bg-[#151C26] px-4 py-2.5" aria-live="polite">
          <p className="flex items-center gap-2 text-[12.5px] font-medium text-white/50">
            <span className={`h-2 w-2 rounded-full ${draftStatus === "saving" ? "animate-pulse bg-[#00E5FF]" : draftStatus === "saved" ? "bg-[#10B981]" : draftStatus === "error" ? "bg-[#EF4444]" : "bg-white/20"}`} aria-hidden="true" />
          ...
          {draftError ? <span className="text-[12px] font-medium text-[#FCA5A5]">{draftError}</span> : null}
        </div>
        ...
        <form onSubmit={onSubmit} noValidate className="mt-3 space-y-5" aria-label="Submit safety report">
          ...
          {confirming ? (
            <div className="rounded-2xl border border-[rgba(0,229,255,0.18)] bg-[rgba(0,229,255,0.06)] p-5" role="group" aria-label="Confirm submission">
              <p className="text-[15px] font-bold text-white">Ready to submit?</p>
              <p className="mt-1 text-[13.5px] leading-relaxed text-white/60">Please review your observation before sending it to the HSE team.</p>
                  ...
                  return (
                    <label
                      ...
                      className={`cursor-pointer rounded-xl border p-4 transition-colors ${
                        selected ? "border-[#00E5FF] bg-[rgba(0,229,255,0.08)] ring-1 ring-[#00E5FF]" : "border-white/5 bg-white/[0.02] hover:border-white/15 hover:bg-white/[0.04]"
                      }`}
                    >
                      <span className="flex items-start gap-2">
                        <input
                          ...
                          className="mt-1 h-5 w-5 accent-[#00E5FF]"
                        />
                        <span>
                          <span className="block text-[14px] font-bold text-white">{o.label}</span>
                          <span className="mt-0.5 block text-[12.5px] leading-relaxed text-white/45">{o.hint}</span>
                        </span>
                      </span>
                    </label>
                  );
          ...
          <Card title="Where and When" subtitle="Location and timing of the observation.">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div data-error={Boolean(errors.site)}>
                ...
                {sitesError ? <p className="mt-1 text-[13px] text-[#FCD34D]">Site list could not be loaded. Please try again later.</p> : null}
              </div>
              ...
              <div className="sm:col-span-2">
                <label htmlFor="worker-equipment" className={labelCls}>Equipment / Asset <span className="font-normal normal-case tracking-normal text-white/30">(optional)</span></label>
                <input
                  id="worker-equipment"
                  type="text"
                  value={equipment}
          ...
          <Card title="What Happened" subtitle="Factual details of what you observed.">
            <div data-error={Boolean(errors.description)}>
              ...
              />
              <p id="worker-description-help" className="mt-1.5 text-[13px] text-white/40">
