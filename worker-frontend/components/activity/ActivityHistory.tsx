  ...
  return (
    <div>
      {loading ? (
        ...
        <div className="rounded-[12px] border border-[rgba(239,68,68,0.20)] bg-[rgba(239,68,68,0.08)] px-3 py-3 text-center">
          <p className="text-[13px] font-semibold text-[#FCA5A5]">Unable to load activity history.</p>
          <button onClick={() => load(1, false)} className="button-secondary mt-2 px-3 py-1 text-[12px]">Retry</button>
        </div>
      ) : items.length === 0 ? (
        <p className="text-[13px] text-white/40">Activity history is not available for this report.</p>
      ) : (
        <>
              ...
              return (
                <li key={ev.id} className="timeline-item">
                  ...
                  <span className="min-w-0">
                    <span className="block font-display text-[13px] font-semibold text-white">{EVENT_LABELS[ev.eventType] ?? ev.eventType}</span>
                    {summary ? <span className="block text-[12px] text-white/45">{summary}</span> : null}
                    {typeof meta.comment === "string" && meta.comment ? (
                      <span className="mt-1 block whitespace-pre-wrap break-words rounded-[8px] border border-white/5 bg-white/[0.02] px-2 py-1.5 text-[12px] leading-relaxed text-white/70">
                        {meta.comment}
                      </span>
                    ) : null}
                    <span className="block font-mono text-[11px] text-white/30">{actorLabel(ev, currentUserId)} · {formatTime(ev.createdAt)}</span>
                  </span>
                </li>
              );
            })}
