  ...
  return (
    <main className="page-canvas">
      ...
      <div className="mx-auto w-[min(100%-2rem,1100px)] space-y-5 py-6">
        <Link href="/worker/reports" className="inline-flex items-center gap-1 text-sm font-semibold text-[#00E5FF] hover:underline">
        ...
        {!user || loading || !report ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
              <div className="space-y-5 lg:col-span-3">
                ...
                <Card title="Description" subtitle="Your original account — kept unchanged">
                  <blockquote className="whitespace-pre-wrap text-[15px] leading-relaxed text-white">
                    {report.description}
                  </blockquote>
                </Card>
              </div>
                  ...
                  <ol className="space-y-0">
                    {timeline.map((s, i) => (
                      <li key={s.label} className="relative flex gap-3 pb-5 last:pb-0">
                        {i < timeline.length - 1 ? (
                          <span className="absolute left-[7px] top-5 h-[calc(100%-1rem)] w-px skeleton" aria-hidden="true" />
                        ) : null}
                        <span
                          className={`mt-1 h-3.5 w-3.5 shrink-0 rounded-full border-2 ${
                            s.done ? "border-green-700 bg-green-700" : s.current ? "border-[#00E5FF] bg-[#131B24]" : "border-slate-300 bg-[#131B24]"
                          }`}
                          aria-hidden="true"
                        />
                        <span>
                          <span className="block text-sm font-semibold text-white">
                            {s.label}
                            {s.current ? <span className="ml-2 text-xs font-medium text-[#00E5FF]">· current</span> : null}
                          </span>
                          <span className="block text-[13px] text-[#94A3B8]">{s.detail}</span>
                        </span>
                      </li>
                    ))}
                  </ol>
                  ...
                  {!review ? (
                    ...
                    <dl className="space-y-2">
                      <div className="flex items-center justify-between gap-3">
                        <dt className="text-[11px] font-bold uppercase tracking-wide text-[#94A3B8]">Decision</dt>
                        <dd>{review.hseDecision ? <DecisionBadge value={review.hseDecision} /> : <span className="text-sm text-slate-400">—</span>}</dd>
                      </div>
                      {review.comment ? (
                        <div>
                          <dt className="text-[11px] font-bold uppercase tracking-wide text-[#94A3B8]">HSE comment</dt>
                          <dd className="mt-0.5 whitespace-pre-wrap text-sm text-slate-800">{review.comment}</dd>
                        </div>
                      ) : null}
                      {review.reviewedAt ? (
                        <div>
                          <dt className="text-[11px] font-bold uppercase tracking-wide text-[#94A3B8]">Reviewed</dt>
            ...
            <Card title="AI Assessment" subtitle="Advisory assessment — you cannot modify it.">
              ...
              <p className="mt-3 border-t border-white/[0.05] pt-3 text-[13px] text-[#94A3B8]">
