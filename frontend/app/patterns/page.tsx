            ...
            <div className="overflow-x-auto">
              <table className="data-table min-w-[760px]">
                ...
                <tbody>
                  {items.map((p) => (
                    <tr key={p.id}>
                      <td className="text-[13.5px] font-semibold text-white">{p.name}</td>
                      <td className="font-mono text-[12px] text-[#94A3B8]">{p.code ?? "—"}</td>
                      <td className="max-w-[360px] truncate text-[13px] text-[#94A3B8]" title={p.description ?? ""}>
                        {p.description ?? <span className="text-[#94A3B8]">—</span>}
                      </td>
                      <td>
                        <StatusBadge value={p.status} />
                      </td>
                      <td>
                        <Link href={`/patterns/${p.id}`} className="text-[12.5px] font-semibold text-[#00E5FF] hover:underline">
                          View →
                        </Link>
                      </td>
                    </tr>
