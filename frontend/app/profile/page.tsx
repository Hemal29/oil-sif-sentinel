...
function Field({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="border-b border-white/[0.06] py-3 last:border-0">
      ...
      <dd className={`mt-1 text-[13.5px] text-white ${mono ? "font-mono" : ""}`}>{value}</dd>
    </div>
  );
}
        ...
        {error && !user ? (
          ...
          <>
            <section className="panel overflow-hidden">
              <div className="flex items-center gap-4 border-b border-white/[0.06] bg-white/[0.04] px-6 py-5">
                <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#0F172A] text-[20px] font-bold text-white">
                  {user.name.charAt(0).toUpperCase()}
                </span>
                <div className="min-w-0">
                  <h2 className="truncate text-[18px] font-bold tracking-tight text-white">{user.name}</h2>
                  <p className="truncate text-[13px] text-[#64748B]">{user.email}</p>
                </div>
            ...
            <section className="panel">
              <div className="panel-header">
                <div className="flex items-center gap-2.5">
                  <ShieldIcon className="h-4 w-4 text-[#64748B]" />
                  <h2 className="section-title">Access & Responsibility</h2>
                </div>
              </div>
              <div className="panel-body">
                <ul className="space-y-2.5 text-[13px] leading-relaxed text-[#94A3B8]">
                  <li className="flex gap-2.5">
                    <UserIcon className="mt-0.5 h-4 w-4 shrink-0 text-[#94A3B8]" />
                    Your review decisions are recorded permanently in the report audit trail.
                  </li>
                  <li className="flex gap-2.5">
                    <ShieldIcon className="mt-0.5 h-4 w-4 shrink-0 text-[#94A3B8]" />
