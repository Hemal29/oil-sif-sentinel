...
export function fetchReviews(filters: { page?: number; limit?: number; hseDecision?: string; reportId?: string } = {}): Promise<{ items: import("@/types/review").Review[]; pagination: import("@/types/report").Pagination }> {
  return data(api.get("/reviews", { params: filters }));
}
...
export function fetchPatterns(q: MasterQuery = {}): Promise<MasterListResponse<PatternItem>> {
  return data(api.get("/patterns", { params: { ...q, limit: q.limit ?? 50 } }));
}
...
export function fetchReportAudit(
  id: string,
  params?: { page?: number; limit?: number }
): Promise<{ items: AuditEventItem[]; pagination: { page: number; limit: number; total: number; totalPages: number } }> {
...
export interface ActionCenterResponse {
  items: Array<{
    ...
    aiAnalysis: { route: string | null; riskScore: number | null; sifPotential: boolean | null; priority: string | null; analysisStatus: string | null } | null;
    status: string;
    createdAt: string;
  }>;
  pagination: { page: number; limit: number; total: number; totalPages: number };
  summary: { pendingReview: number; priority: number; uncertainOrAbstain: number; needsInformation: number };
}

export function fetchActionCenter(params: Record<string, string | number | undefined>): Promise<ActionCenterResponse> {
  return data(api.get("/reviews/action-center", { params }));
}
