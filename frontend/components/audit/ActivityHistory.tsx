...
function EventDetail({ ev }: { ev: AuditEventItem }) {
  ...
  return (
    <span className="min-w-0">
      <span className="block text-[13px] font-semibold text-white">{EVENT_LABELS[ev.eventType] ?? ev.eventType}</span>
      <span className="block text-[12px] text-[#627D98]">Actor: {actorLabel(ev)} · {formatTime(ev.createdAt)}</span>
      {chips.length > 0 ? <span className="block text-[12px] text-[#627D98]">{chips.join(" · ")}</span> : null}
      {typeof meta.comment === "string" && meta.comment ? (
        <span className="mt-1 block whitespace-pre-wrap break-words rounded-[4px] border border-white/[0.06] bg-white/[0.04] px-2 py-1.5 text-[12px] leading-relaxed text-[#CBD5E1]">
          {meta.comment}
        </span>
      ) : null}
    </span>
