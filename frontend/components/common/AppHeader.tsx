...
export function AppHeader({ title }: { title: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0e2238] text-slate-200 shadow-[0_1px_0_rgba(255,255,255,0.06)_inset,0_2px_8px_rgba(2,6,23,0.25)]">
      {/* Brand row */}
      <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4 px-4 pt-3 sm:px-6">
        <Link href="/dashboard" className="flex min-w-0 items-center gap-3" aria-label="OIL SIF Sentinel — Dashboard">
          ...
          <span className="min-w-0">
            <span className="block truncate text-[11px] font-bold uppercase tracking-[0.18em] text-sky-300/90">
              Oil SIF Sentinel
            </span>
          ...
          {user ? (
            <div className="hidden items-center gap-2 md:flex" aria-label="Signed-in user">
              ...
              <span className="text-right leading-tight">
                ...
                <span className="block text-[11px] font-medium uppercase tracking-wide text-sky-300/80">
                  {user.role.replace("_", " ")}
                </span>
              </span>
            </div>
        ...
        {NAV.map((n) => {
          ...
          return (
            ...
            >
              {n.label}
              {isActive ? (
                <span className="absolute inset-x-3 -bottom-[1px] h-0.5 rounded-full bg-sky-400" aria-hidden="true" />
              ) : null}
            </Link>
          );
        })}
        ...
        <nav className="border-t border-white/10 px-4 py-2 lg:hidden" aria-label="Mobile navigation">
          ...
          {user ? (
            <p className="px-3 py-2 text-xs text-slate-400">
              ...
              <span className="uppercase tracking-wide text-sky-300/80">{user.role.replace("_", " ")}</span>
            </p>
          ) : null}
        </nav>
      )}
