...
/* Restrained enterprise palette: blue/gray first, muted semantics only where
   safety meaning requires it (rejected, confirmed/closed, needs-info). */
...
function BaseBadge({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full border px-2.5 py-[3px] text-[11px] font-semibold uppercase tracking-wide",
        className
      )}
    >
      {children}
...
export function SifBadge({ value }: { value: boolean | string | null }) {
  ...
  if (typeof value === "string") {
    ...
    if (lower === "yes" || lower === "true" || lower === "sif") return <BaseBadge className="badge-info">SIF</BaseBadge>;
    if (lower === "no" || lower === "false" || lower === "non-sif") return <BaseBadge className="badge-neutral">Non-SIF</BaseBadge>;
    if (lower.includes("insufficient")) return <BaseBadge className="badge-neutral">Insufficient</BaseBadge>;
  }
  // boolean path
  return (
    <BaseBadge className={value ? "badge-info" : "badge-neutral"}>
      {value ? "SIF" : "Non-SIF"}
    </BaseBadge>
  );
}
