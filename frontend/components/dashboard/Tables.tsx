...
const TH = "whitespace-nowrap px-3 py-2.5 text-left text-[11px] font-bold uppercase tracking-wide text-[#94A3B8] bg-[#0B0E14] border-b border-white/[0.06]";
const TD = "whitespace-nowrap px-3 py-2.5 text-[13px] text-white border-b border-white/[0.05]";
...
function ActionLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="whitespace-nowrap text-[13px] font-semibold text-[#00E5FF] hover:text-[#33EAFF] hover:underline">
      {children}
    </Link>
  );
}
      ...
      {items.length === 0 ? (
        ...
        <ScrollTable>
          <table className="w-full min-w-[520px]">
            ...
            <tbody>
              {items.map((s) => (
                <tr key={s.siteId} className="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.04]">
                  <td className={TD}>
                    <span className="font-medium text-white">{s.siteName}</span>
                    {s.siteCode ? <span className="ml-2 text-xs tabular-nums text-[#64748B]">{s.siteCode}</span> : null}
                  </td>
      ...
      {items.length === 0 ? (
        ...
        <ScrollTable>
          <table className="w-full min-w-[560px]">
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.ruleId} className="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.04]">
                  <td className={TD}>
                    <span className="font-semibold text-white">{r.ruleCode}</span>
                    <span className="ml-2 text-[#94A3B8]">{r.ruleName}</span>
                  </td>
                  <td className={TD}>
                    {r.isPrototype ? (
                      <span className="rounded-full border border-white/[0.08] bg-white/[0.06] px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[#94A3B8]">
                        Prototype
                      </span>
                    ) : (
                      <span className="rounded-full border border-[rgba(0,229,255,0.22)] bg-[rgba(0,229,255,0.10)] px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[#00E5FF]">
                        Official
                      </span>
                    )}
                  </td>
      ...
      {items.length === 0 ? (
        ...
        <ScrollTable>
          <table className="w-full min-w-[920px]">
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.analysisId} className="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.04]">
                  <td className={TD}>
                    <Link href={`/reports/${r.reportId}`} className="font-semibold text-[#00E5FF] hover:underline">
                      {r.reportNumber}
                    </Link>
                  </td>
      ...
      {items.length === 0 ? (
        ...
        <ScrollTable>
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.reportId} className="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.04]">
                  <td className={TD}>
                    <Link href={`/reports/${r.reportId}`} className="font-semibold text-[#00E5FF] hover:underline">
                      {r.reportNumber}
                    </Link>
      ...
      {items.length === 0 ? (
        ...
        <ScrollTable>
          <table className="w-full min-w-[320px]">
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.name} className="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.04]">
                  <td className={TD}>
