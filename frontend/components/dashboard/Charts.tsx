...
const CYAN_SOFT = "rgba(0,229,255,0.10)";
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="min-w-36 rounded-[10px] border border-white/[0.08] bg-[#161F2B] px-3 py-2 shadow-[0_12px_32px_rgba(0,0,0,0.55)] backdrop-blur-[8px]">
      {label !== undefined ? (
        <p className="mb-1 text-xs font-semibold text-white">{label}</p>
      ) : null}
      {payload.map((entry: any, i: number) => (
        <p key={i} className="flex items-center justify-between gap-4 text-xs tabular-nums text-[#94A3B8]">
          <span>
            <span
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={points} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              <Area
                ...
                fillOpacity={0.12}
                dot={false}
                activeDot={{ r: 3, stroke: "#00E5FF", strokeWidth: 2, fill: "#0B0E14" }}
              />
              <Area
                type="monotone"
                dataKey="sifReports"
                ...
                fillOpacity={0.10}
                dot={false}
                activeDot={{ r: 3, stroke: "#F59E0B", strokeWidth: 2, fill: "#0B0E14" }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="relative w-full max-w-[220px]" style={{ height }}>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="text-[11px] font-semibold uppercase tracking-wide text-[#64748B]">{totalLabel}</p>
        </div>
      </div>
        ...
        {entries.map((e) => (
          <li key={e.key} className="flex items-center justify-between gap-3 text-[13px]">
            <span className="flex min-w-0 items-center gap-2 text-[#94A3B8]">
              <span className="h-2.5 w-2.5 shrink-0 rounded-sm" style={{ background: colors[e.key] ?? "#94a3b8" }} />
              <span className="whitespace-normal">{e.name}</span>
            </span>
            <span className="shrink-0 font-bold tabular-nums text-white">{e.value.toLocaleString()}</span>
          </li>
        ))}
      </ul>
    </div>
          ...
          {entries.map((e) => (
            <li key={e.key}>
              <div className="mb-1.5 flex items-start justify-between gap-3 text-[13px]">
                <span className="min-w-0 font-medium text-[#CBD5E1]">{e.name}</span>
                <span className="shrink-0 font-semibold tabular-nums text-white">
                  {e.value.toLocaleString()}
                  <span className="ml-2 font-medium text-[#64748B]">{((e.value / total) * 100).toFixed(0)}%</span>
                </span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-white/[0.06] border border-white/[0.04]" role="img" aria-label={`${e.name}: ${e.value} reports`}>
                <div className="h-full rounded-full bg-[#00E5FF]" style={{ width: `${(e.value / max) * 100}%` }} />
              </div>
            </li>
          ))}
        </ul>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={top} layout="vertical" margin={{ top: 0, right: 24, left: 8, bottom: 0 }} barCategoryGap="24%">
              ...
              <Legend formatter={(v) => <LegendText value={v} />} iconSize={10} />
              <Bar dataKey="reportCount" name="Reports" fill="#00E5FF" barSize={12} radius={[0, 3, 3, 0]} />
              <Bar dataKey="sifCount" name="SIF (AI)" fill="#F59E0B" barSize={12} radius={[0, 3, 3, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
